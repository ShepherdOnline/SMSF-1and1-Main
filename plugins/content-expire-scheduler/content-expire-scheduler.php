...
add_filter( 'plugin_action_links_' . plugin_basename(__FILE__), 'ces_plugin_actions' );

function ces_plugin_actions( $links ) {
   $links[] = '<a href="'. esc_url( get_admin_url(null, 'options-general.php?page=ces-options') ) .'">Settings</a>';
   return $links;
}
...
function rl_ces_expiry_content($content, $post_id = 0) {
  global $post;
  ...
  $ces_ex_msg_color = get_option('ces_expiry_msg_color');
  $ces_ex_msg_bg_color = get_option('ces_expiry_msg_bg');
  $ces_ex_msg_sho = get_option('ces_ex_msg_sho');

  if (!empty($ces_ex_msg_color)){
    $ces_msg_color_print = 'color: '.$ces_ex_msg_color.';';
  }
  if (!empty($ces_ex_msg_bg_color)){
    $ces_ex_msg_bg_color = 'background-color: '.$ces_ex_msg_bg_color.';padding: 10px;';
  }

  if (empty($expires_msg)) {
    $expiry_m = '<p class="ces-msg" style="'.$ces_msg_color_print.$ces_ex_msg_bg_color.'">'.$ces_expiry_message.'</p>';
  }else{
    $expiry_m = '<p class="ces-msg" style="'.$ces_msg_color_print.$ces_ex_msg_bg_color.'">'.$expires_msg.'</p>';
  }

  if ($keep_content == 1) {
    if ($ces_ex_msg_sho == 0) {
      $ces_content = $expiry_m . $content;
    }else{
      $ces_content = $content . $expiry_m;
    }
    return $ces_content;
  }else {
    if( rl_ces_is_content_expired( $post_id ) ) {
...
function rl_ces_script_setting() {
  wp_enqueue_style( 'wp-color-picker');
  wp_enqueue_script( 'wp-color-picker');
  wp_enqueue_script( 'rl_ces_script', RLCES_URL . '/assets/js/wp-ces-color.js', array( 'jquery' ), RLCES_VERSION, true );
}
add_action( 'admin_enqueue_scripts', 'rl_ces_script_setting' );
