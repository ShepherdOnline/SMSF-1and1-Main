...
class EMEFS {
   ...
   function processForm() {
      ...
      if (isset($_POST['event']) && isset($_POST['new-event']) && wp_verify_nonce($_POST['new-event'], 'action_new_event')) {
         ...
         if(!$time_start){
            $emefs_event_errors['event_start_time'] = __('Check the start\'s date and time', 'events-made-easy-frontend-submit');
         }

         if(!$time_end){
            $emefs_event_errors['event_end_time'] =  __('Check the end\'s date and time', 'events-made-easy-frontend-submit');
         }

         if($time_start && $time_end && $time_start > $time_end){
            $emefs_event_errors['event_time'] =  __('The event\'s end must be <strong>after</strong> the event\'s start', 'events-made-easy-frontend-submit');
         }

         if ( !isset($event_data['event_notes']) || empty($event_data['event_notes']) ) { 
            $emefs_event_errors['event_notes'] = __('Please enter a description for the event', 'events-made-easy-frontend-submit'); 
         }
    ...
    */
   public static function attribute($field_id = false,$type = 'text',$more='') {
      if (!$field_id)
         return false;
