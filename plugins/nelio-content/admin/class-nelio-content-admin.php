...
class Nelio_Content_Admin {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(
			...
			array( 'jquery', 'backbone', 'debugjs', 'moment-timezone-js', 'ncselect2-i18n', 'latinize', 'store-js', 'jquery-ui-datepicker', 'jquery-ui-dialog', 'jquery-ui-tooltip', 'twitter-text' ),
			$this->version,
			false
		);
		...
		if ( $this->is_current_screen( '_nc-settings' ) ) {
			...
			wp_enqueue_script(
				...
				array( $this->plugin_name, $settings->get_generic_script_name() ),
				$this->version,
				'all'
			);
	...
	private function add_meta_boxes() {
		...
		if ( nc_is_subscribed_to( 'personal-plan' ) ) {
		...
		foreach ( $screen_options as $screen_option ) {
			...
			switch ( $screen_option ) {
				...
				case '_nc-post-with-featured-image':
					return in_array( $screen->id, $post_types ) && post_type_supports( $screen->id, 'thumbnail' );
	...
	public function is_edit_screen() {
		...
		$screen = get_current_screen();
		$post_types = get_post_types();
		return in_array( $screen->id, $post_types );
	...
	public function is_a_nelio_content_page() {
		...
		$post_types = get_post_types();
		if ( in_array( $screen->id, $post_types ) && post_type_supports( $screen->id, 'thumbnail' ) ) {
			return true;
		}//end if
	...
	private function generate_js_object() {
		...
		$result = array(
			...
			'subscriptionPlan' => nc_get_subscription_plan(),
